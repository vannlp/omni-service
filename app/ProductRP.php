 ...
 * @package App
 */
class ProductRP extends BaseModel
{
    /**
     * @var
     */
    ...
    public function priceDetail($product = null)
    {
        ...
        if (!empty($product)) {
            ...
            //         ->whereDate('prices.from', '<=', $now)
            //         ->whereDate('prices.to', '>=', $now)
            //         ->orderByDesc('order');
            //     $price_detail->Where(function ($q) {
            //         $q->whereNull('ward_code')->whereNull('district_code')->whereNull('city_code');
            //         $q->orWhere(
            //             [
            //                 [DB::raw("CONCAT(',',city_code,',')"), 'like', "%," . TM::getCurrentCityCode() . ",%"],
            //                 [DB::raw("CONCAT(',',district_code,',')"), 'like', "%," . TM::getCurrentDistrictCode() . ",%"],
            //                 [DB::raw("CONCAT(',',ward_code,',')"), 'like', "%," . TM::getCurrentWardCode() . ",%"],
            //             ]
            //         );
            //         $q->orWhere(
            //             [
            ...
            //                 [DB::raw("CONCAT(',',city_code,',')"), 'like', "%," . TM::getCurrentCityCode() . ",%"],
            //             ]
            //         )->whereNull('ward_code')->whereNull('district_code');
            //     });
            //     $price_detail = $price_detail->first();
            //     return $price_detail;
            // } else {
            //     $now = date('Y-m-d', time());
            //     $price_detail = PriceDetail::model()->where('product_id', $product->id)->join('prices', 'prices.id', '=', 'price_details.price_id')
    ...
    // public function priceDetail2($area = null)
    // {
    //     $group_id = app('request')->input('customer_group_id', DataUser::getInstance()->groupId);

    //     $a = !empty($area) ? array_pluck(json_decode($area), 'code') : [];


    //     $now = date('Y-m-d', time());
    //     $price_detail = PriceDetail::model()->join('prices', 'prices.id', '=', 'price_details.price_id')
    //     ->where(DB::raw("CONCAT(',',group_ids,',')"), 'like', "%,$group_id,%")
    ...
    //     ->orderByDesc('order');
    //     foreach($a as $code){
    //         $price_detail->orWhere(function($q) use($code){
    //             $q->where(DB::raw("CONCAT(',',sale_area,',')"), 'like', "%,$code,%");
    //         });
    //     }
