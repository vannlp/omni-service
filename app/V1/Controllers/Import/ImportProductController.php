...
class ImportProductController
{
                ...
                foreach ($sheet->getRowIterator() as $index => $row) {
                    ...
                    if (array_filter($row)) {
                        ...
                        $valids[] = [
                            ...
                            'group_code'                => !empty($list_group) ? json_encode($list_group) : [],
                            ...
                            'slug'                      => !empty($row[51]) ? $row[51] : Str::slug($row[1])
                        ];
                    }
                }
    ...
    private function startImport($data, $codeUsed = [])
    {
        ...
        DB::beginTransaction();
        $queryHeader = "INSERT INTO `products` (" .
            "`code`," .
            "`name`, " .
            "`price`," .
            "`unit_id`," .
        ...
        foreach ($data as $datum) {
            $queryContent .=
                ...
                ($datum['specification_id'] ? $pdo->quote($datum['specification_id']) : "null") . "," .
                ($datum['tax'] ? $pdo->quote($datum['tax']) : "null") . "," .
                ($datum['status'] ? $pdo->quote($datum['status']) : "null") . "," .
                $pdo->quote($datum['publish_status']) . "," .
                $pdo->quote($datum['type']) . "," .
                $pdo->quote($datum['is_cool']) . "," .
                ($datum['short_description'] ? $pdo->quote($datum['short_description']) : "null") . "," .
                ($datum['description'] ? $pdo->quote($datum['description']) : "null") . "," .
                ($datum['weight'] ? $pdo->quote($datum['weight']) : "null") . "," .
                ($datum['weight_class'] ? $pdo->quote($datum['weight_class']) : "null") . "," .
                ($datum['width'] ? $pdo->quote($datum['width']) : "null") . "," .
                ($datum['height'] ? $pdo->quote($datum['height']) : "null") . "," .
                ($datum['length'] ? $pdo->quote($datum['length']) : "null") . "," .
                ($datum['length_class'] ? $pdo->quote($datum['length_class']) : "null") . "," .
                ($datum['tags'] ? $pdo->quote($datum['tags']) : "null") . "," .
                ($datum['category_ids'] ? $pdo->quote($datum['category_ids']) : "null") . "," .
                ($datum['thumbnail'] ? $pdo->quote($datum['thumbnail']) : "null") . "," .
                ($datum['gallery_images'] ? $pdo->quote($datum['gallery_images']) : "null") . "," .
                ($datum['p_sku'] ? $pdo->quote($datum['p_sku']) : "null") . "," .
                ($datum['sku_name'] ? $pdo->quote($datum['sku_name']) : "null") . "," .
                ($datum['sku_standard'] ? $pdo->quote($datum['sku_standard']) : "null") . "," .
                ($datum['custom_date_updated'] ? $pdo->quote($datum['custom_date_updated']) : "null") . "," .
                ($datum['age_id'] ? $pdo->quote($datum['age_id']) : "null") . "," .
                ($datum['expiry_date'] ? $pdo->quote($datum['expiry_date']) : "null") . "," .
                ($datum['manufacture_id'] ? $pdo->quote($datum['manufacture_id']) : "null") . "," .
                ($datum['area_id'] ? $pdo->quote($datum['area_id']) : "null") . "," .
                ($datum['area_name'] ? $pdo->quote($datum['area_name']) : "null") . "," .
                ($datum['capacity'] ? $pdo->quote($datum['capacity']) : "null") . "," .
                ($datum['brand_id'] ? $pdo->quote($datum['brand_id']) : "null") . "," .
                ($datum['child_brand_id'] ? $pdo->quote($datum['child_brand_id']) : "null") . "," .
                ($datum['child_brand_name'] ? $pdo->quote($datum['child_brand_name']) : "null") . "," .
                ($datum['cat'] ? $pdo->quote($datum['cat']) : "null") . "," .
                ($datum['subcat'] ? $pdo->quote($datum['subcat']) : "null") . "," .
                ($datum['cadcode'] ? $pdo->quote($datum['cadcode']) : "null") . "," .
                ($datum['cad_code_subcat'] ? $pdo->quote($datum['cad_code_subcat']) : "null") . "," .
                ($datum['cad_code_brand'] ? $pdo->quote($datum['cad_code_brand']) : "null") . "," .
                ($datum['cad_code_brandy'] ? $pdo->quote($datum['cad_code_brandy']) : "null") . "," .
                ($datum['division'] ? $pdo->quote($datum['division']) : "null") . "," .
                ($datum['source'] ? $pdo->quote($datum['source']) : "null") . "," .
                ($datum['packing'] ? $pdo->quote($datum['packing']) : "null") . "," .
                ($datum['p_type'] ? $pdo->quote($datum['p_type']) : "null") . "," .
                ($datum['p_attribute'] ? $pdo->quote($datum['p_attribute']) : "null") . "," .
                ($datum['p_variant'] ? $pdo->quote($datum['p_variant']) : "null") . "," .
                ($datum['group_code'] ? $pdo->quote($datum['group_code']) : "null") . "," .
                ($datum['group_code_ids'] ? $pdo->quote($datum['group_code_ids']) : "null") . "," .
                ($datum['property_variant_root'] ? $pdo->quote($datum['property_variant_root']) : "null") . "," .
                ($datum['property_variant_root_ids'] ? $pdo->quote($datum['property_variant_root_ids']) : "null") . "," .
                ($datum['property_variant_ids'] ? $pdo->quote($datum['property_variant_ids']) : "null") . "," .
                ($datum['meta_title'] ? $pdo->quote($datum['meta_title']) : "null") . "," .
                ($datum['meta_description'] ? $pdo->quote($datum['meta_description']) : "null") . "," .
                ($datum['meta_keyword'] ? $pdo->quote($datum['meta_keyword']) : "null") . "," .
                ($datum['meta_robot'] ? $pdo->quote($datum['meta_robot']) : "null") . "," .
