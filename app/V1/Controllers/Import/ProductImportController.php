                ...
                foreach ($sheet->getRowIterator() as $index => $row) {
                    ...
                    if (array_filter($row)) {
                        ...
                        $valids[] = [
                            'code'              => $row[0],
                            'name'              => $row[1],
                            'category_ids'      => !empty($row[2]) ? $existedChk['Category'][trim(strtoupper($row[2]))] : null,
                            'order'             => !empty($row[3]) ? $row[3] : null,
                            'description'       => !empty($row[4]) ? $row[4] : null,
                            'short_description' => !empty($row[5]) ? $row[5] : null,
                            'sku'               => !empty($row[6]) ? $row[6] : null,
                            'weight_class'      => !empty($row[7]) ? $row[7] : null,
                            'unit_id'           => !empty($row[8]) ? $existedChk['Unit'][trim(strtoupper($row[8]))] : null,
                            'width'             => !empty($row[9]) ? $row[9] : null,
                            'length'            => !empty($row[10]) ? $row[10] : null,
                            'qty'               => !empty($row[11]) ? $row[11] : null,
                            'weight'            => !empty($row[12]) ? $row[12] : null,
                            'is_active'         => 1,
                        ];
                        // Put error
                        if ($errorDetail) {
    ...
    private function startImport($data, $codeUsed = [])
    {
        ...
        foreach ($data as $datum) {
            $queryContent .=
                ...
                $pdo->quote(Str::slug($datum['name'])) . "," .
                $pdo->quote($datum['category_ids']) . "," .
                ($datum['order'] ? $pdo->quote($datum['order']) : "null") . "," .
                ($datum['description'] ? $pdo->quote($datum['description']) : "null") . "," .
                $pdo->quote($datum['short_description']) . "," .
                ($datum['sku'] ? $pdo->quote($datum['sku']) : "null") . "," .
                ...
                ($datum['qty'] ? $pdo->quote($datum['qty']) : "null") . "," .
