            ...
            foreach ($input['details'] as $key => $item) {
                ...
                if (empty($inventoryDetail)) {
                    $param                = [
                        ...
                        "variant_id"     => array_get($item, 'variant_id', null),
                    ...
                    $inventoryDetail->variant_id     = array_get($item, 'variant_id', null);
...
//                        $inventoriesBatch      = Batch::find($detail['batch_id']);
                        $param                 = [
                            ...
                            "variant_id"     => array_get($detail, 'variant_id', null),
                            "product_code"   => $inventoriesProduct->code,
                            "product_name"   => $inventoriesProduct->name,
			...
			$companyId = TM::getCurrentCompanyId();
                        if (empty($warehouseDetails)){
                            throw new \Exception(Message::get("V052", "ID: #{$inventory->id} - CompanyID: {$companyId} - ProductID: {$detail['product_id']} - WarehouseID: {$detail['warehouse_id']} - BatchID: {$detail['batch_id']} - UnitID: {$detail['unit_id']}"));
                        }
                        if ($warehouseDetails->quantity < $detail['quantity']) {
                            throw new \Exception(Message::get("V053", "ID: #{$inventory->id} - ProductID: {$detail['product_id']} - WarehouseID: {$warehouseDetails->warehouse_id} - BatchID: {$warehouseDetails->batch_id} - UnitID: {$detail['unit_id']}"));
                        } else {
                            $warehouseDetails->quantity   -= $detail['quantity'];
                            $warehouseDetails->updated_at = date('Y-m-d H:i:s', time());
                            $warehouseDetails->updated_by = 1234;
