...
class TempOrderExportCronCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'temp_order_export:cron';
    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Temp Order Export command';
    ...
    public function handle()
    {
        ...
        // $date_run = date('Y-m-d H:i:s', strtotime($process->date_run));
        // $reset = (new DateTime())->modify('-1 hour')->format('Y-m-d H:i:s');
        // $process =  DB::table('check_process_cron_order')->select('status', 'date_run')
        //     ->where('date_run', '<', $reset)->where('name','=', 'Order Export Cron')->first();
        $check =  DB::table('check_process_cron_order')->where('name', 'Order Export Cron')->where('status', 1)->first();
        if (!empty($check)) {
            $this->info('Process is running');
            return;
        }

        DB::table('check_process_cron_order')->update(['status' => 1, 'name' => 'Temp Order Export Cron', 'date_run' =>  date('Y-m-d H:i:s')]);
        ...
        $temp_order = TempOrderExportExcelRP::all();
        if (empty($temp_order)) {
            DB::table('check_process_cron_order')->update(['status' => 0]);
            return [];
        }
        try {
            // dd(json_encode($temp_order));
            foreach ($temp_order as $temp) {
                $orders = OrderRP::with(['details', 'details.product.unit', 'details.product', 'vpvirtualaccount', 'store', 'customer', 'distributor', 'distributor.profile', 'distributor.profile.city', 'distributor.profile.district', 'distributor.profile.ward'])
                    ->where('code', $temp->order_code)
                    ->get();
                if (empty($orders) || json_encode($orders) == '[]') {
                    DB::table('check_process_cron_order')->update(['status' => 0]);
                    return [];
                }
                foreach ($orders as $key => $order) {
                    ...
                    $dis_ship   = 0;
                    $feeship    = 0;
                    if (!empty($order->total_info) && $order->total_info != "null") {
                    ...
                    foreach ($details as $detail) {
                        ...
                        $dataOrder[]     = [
                            ...
                            'total_price_product'           => ($detail->price * $detail->qty) - ($detail->discount * $detail->qty) ?? 0,
                            ...
                            'total_price'                   => $detail->id == 27343 ? 48000 : $order->total_price,
                            ...
                            'special_percentage'            => $detail->id == 27343 ? 10  :( in_array($detail->product_code, $prodcode) ? $special_percentage : ($detail->discount > 0 ? $this->parsePriceByProducts($detail->product_code, ($detail->price * $detail->qty)) : 0)),
                            ...
                            'order_price_CK'        => !empty($order_price_CK) ? $order_price_CK : 0,
                        ...
                        foreach (json_decode($order->free_item) as $free) {
                            foreach ($free->text as $item) {
                                ...
                                $dataOrder[]     = [
                                    ...
                                    // 'price_CK'              0,//=> ($detail->price - ($detail->total/$detail->qty)) > 0 ? $detail->price - ($detail->total/$detail->qty) : 0,
                                    'price_CK'              => 0, //in_array($detail->product_code, $prodcode) ? ($special_percentage/100) * $detail->price :(($detail->price - ($detail->total/$detail->qty)) > 0 ? $detail->price - ($detail->total/$detail->qty) : 0),
                                    // ,0,//
                                    'total_price_product'   => 0,
                                    // 'total_price'           0,//=> $detail->price * $detail['qty'] ?? 0,
                                    'discount'              => 0, //$detail['discount'] ?? 0,
                                    'payment'               => 0, //$detail->total,
                                    'payment_code'          => $order->payment_code,
                                    'payment_method'        => $order->payment_method,
                                    'shipping_method_name'  => $order->shipping_method_name,
                                    'is_freeship'           => $order->is_freeship,
                                    ...
                                    'discount_coupon'       => !empty($dis_coupon) ? $dis_coupon : 0,
                                    'discount_fee_ship'     => !empty($dis_ship) ? $dis_ship : null,
                                    'item_product_code'     => 0, //implode(", ", $pod_gift),
                                    'item_product_name'     => 0, //implode(", ", $pod_name_gift),
                                    'item_product_qty'      => 0, //implode(", ", $pod_qty_gift),
                                    'special_percentage'    => 0, //in_array($detail->product_code, $prodcode) ? $special_percentage : 0,
                                    // 'special_percentage'    0,//=> !empty($promotion) && $promotion->act_sale_type == 'percentage' ? $special_percentage : 0,
            ...
            $this->insertOrderExportExcel($dataOrder);
        } catch (\Exception $e) {
            DB::table('check_process_cron_order')->update(['status' => 0]);
            $this->writeLog('INSERT TEMP EXPORT ORDERS', $e, 'insert or update triggers', $temp_order);
        }
        DB::table('check_process_cron_order')->update(['status' => 0]);
    }
    ...
    public function insertOrderExportExcel($data)
    {
        try {
            if (empty($data)) {
                DB::table('check_process_cron_order')->update(['status' => 0]);
            ...
            $checkDelete = OrderExportExcel::model()->where('order_code', $data[0]['order_code'])->get();
            /** @var \PDO $pdo */
            $pdo = DB::getPdo();
            ...
            if (!empty($checkDelete)) {
                OrderExportExcel::model()->where('order_code', $data[0]['order_code'])->delete();
            }
            ...
            foreach ($data as $key => $value) {
                TempOrderExportExcel::where('order_code', $value['order_code'])->update(['deleted' => 1, 'deleted_by' => TM::getCurrentUserId()]);
                TempOrderExportExcel::where('order_code', $value['order_code'])->delete();
            ...
            // foreach ($data as $key => $value) {
            //     TempOrderExportExcel::where('order_code', $value['order_code'])->delete();
            //     OrderExportExcel::create($value);
            // }
            DB::commit();
        } catch (\Exception $e) {
            DB::table('check_process_cron_order')->update(['status' => 0]);
            $this->writeLog('INSERT TEMP EXPORT ORDERS', $e, 'insert or update triggers', $data);
        }
        DB::table('check_process_cron_order')->update(['status' => 0]);
    }
