                        ...
                        <table class="email-body_inner" align="left" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td class="content-cell">
                                    <h1>Xin chào khách hàng: {{$order->customer_name ?? null}}</h1>
                                    <p>Chúng tôi đã nhận được yêu cầu đặt hàng của bạn vào ngày:
                                        <strong>{{date("d/m/Y", strtotime($order->created_date))}}</strong></p>
                                    <!-- Action -->
                                    {{--                                    <p>&nbsp;</p>--}}
                                    <p>Đính kèm là chi tiết đơn hàng bạn đã đặt:</p>

                                    <p>Mã đơn hàng: <strong>{{$order->code}}</strong></p>
                                    <p>Khách hàng: <strong>{{$order->customer->name ?? null}}</strong></p>
                                    <p>Số điện thoại khách hàng: <strong>{{$order->customer_phone}}</strong></p>
                                    <p>Số điện thoại người nhận hàng: <strong>{{$order->shipping_address_phone}}</strong></p>
                                    <p>Địa chỉ nhận hàng:
                                        <strong>{{\Illuminate\Support\Arr::get($order, "shipping_address", "")}}</strong>
                                    </p>
                                    {{--                                    <p>&nbsp;</p>--}}
                                    ...
                                    <table class="body-action" align="center" width="100%" cellpadding="0"
                                        ...
                                        @if(!$order->promotionTotals->isEmpty())
                                            @foreach($order->promotionTotals as $item)
                                                <tr>
                                                    ...
                                                    <td style="text-align: right">{{number_format($item->value)}}
                                                        đ&nbsp;
                                                    </td>
                                                </tr>
                                            @endforeach
                                        ...
                                        <tr>
                                            ...
                                            <td style="color: red;text-align: right;"><strong>{{number_format($order->total_price)}}đ&nbsp;</strong>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="10" style="text-align: left;">&nbsp;Trong trường hợp cần hỗ
                                                trợ, quý
                                                khách hàng liên hệ số hotline nhà phân phối:
                                                <strong>{{\Illuminate\Support\Arr::get($order, "distributor.phone", "")}}</strong>
                                            </td>
                                        </tr>
                                        </tbody>
                                    </table>
